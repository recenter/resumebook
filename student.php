...
{
    if( $_SESSION['access'] == 0 || $_SESSION['access'] == 2 || $_SESSION['access'] == 3 )
    {
    ...
    if( $_FILES['userfile']['name'] != NULL || $_FILES['userfile']['name'] != '' )
    {
        ...
        if( $bResume )
        {
            ...
            mssql_query("UPDATE students SET resume = '$resumeRename1' WHERE id = '$userid'");
    ...
    else if( $_GET['action'] == 'editprof' )
    {
        if( $_GET['sub'] == '' )
        {
            ...
            $arguscertificate = '';
            $class = '';
            $classeslabels = array( 0 => 'f3230', 1 => 'f3332', 2 => 'f3333', 3 => 'f3334', 4 => 'b3274', 5 => 'f3302', 6 => 'e3439w', 7 => 'e2327' , 8=> 'f4895');
            ...
            <form name="stprof" id="stprof" method="post" action="student.php?action=editprof&amp;sub=submit&amp;id=' . $_GET['id'] . '">
                <table width="100%" class="table_topBorder">
                    ...
                    <th style="border-bottom:1px solid #cccccc;">
                    	<br>
                    	<b><font size="+1">About</font></b><br>
                    	<i>Please indicate which of the following apply to you.</i>
                    </th>
                    <tr>
                        <td><input type="checkbox" name="argustut" value="yes" ' . ( $argus == "1" ? 'checked' : '' ) . ' />&nbsp;&nbsp;&nbsp;Currently taking or have taken the ARGUS class</td>
                    </tr>
                    <tr>
                        <td><input type="checkbox" name="arguscertificate" value="yes" ' . ( $arguscertificate == "1" ? 'checked' : '' ) . ' />&nbsp;&nbsp;&nbsp;ARGUS Certified</td>
                    </tr>
                    <tr>
                        <td>
                            <input type="checkbox" name="undergradOrMBA" value="yes" ' . ( $mba == "1" ? 'checked' : '' ) . ' />&nbsp;&nbsp;&nbsp;MBA Student
                        </td>
                    </tr>';                                   
            ...
            echo '
                    <th style="border-bottom:1px solid #cccccc;">
                    	<br>
                    	<b><font size="+1">Job Preferences & History</font></b>
                    </th>';
            ...
            if( $_SESSION['class'] != 0 )
            {
                echo '
                    <th style="border-bottom:1px solid #cccccc;">
                    	<br>
                    	<b><font size="+1">Student Info</font></b>
                    </th>
                    <tr>
                        <td>
        ...
        else if( $_GET['sub'] == 'submit' )
        {
            ...
            // -1 for placed means "leave the placed flag as it is (i.e., don't updated it)"                        
            updateStudent($_SESSION['id'],$argus,$arguscertificate, $classes/*$_POST['classes']*/, $_POST['class'], $majorIndex, $careertype, $geoprefIndex, $_POST['phone'], $_POST['email'], $permadd, $schooladd, $pl1, $pl2, $pl3, $hidden,$mba,-1/*,$_SESSION['employee']*/);                
        }
    }
    ...
    if( $_SESSION['resume'] != NULL )
    {
        ...
        {
            echo '
            <h4>Please follow this <a href="http://www3.business.uconn.edu/reresume/docs/Resume_template.pdf">template</a> to prepare your resume.<br>For best results, make sure to upload your resume as a <font color = blue> PDF DOCUMENT.</font></h4><br />';    
        ...
        else if( $_GET['action'] == 'delete' )
        {
            ...
            {
                $userid = $_SESSION['id'];        
                mssql_query( "UPDATE students SET resume = NULL WHERE id = '$userid' ");
                updateResume(NULL);// 0 );
